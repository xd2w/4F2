...
cvx_begin sdp
    ...
    variable a2
    variable b2
    minimise(b1 + a2 + b2 + a1)
    LMI2 = a < 0;
    LMI3 = ep > 0;

    LMI4 = [A1'*P +  P*A1 + 2*lambda*P - a1*(C'*C) + ep*eye(n), P*B-C';
            B'*P-C, -b1] < 0;
    LMI5 = [A2'*P +  P*A2 + 2*lambda*P- a1*(C'*C) + ep*eye(n), P*B-C';
            B'*P-C, -b1] < 0;

    LMI1 = [Y*Anonlin' + Anonlin*Y + 2*lambda*Y + ep*eye(3) + Bnonlin*Z + Z'*Bnonlin', Bnonlin - Y*Cnl', Y*Cnl';
            B' - C*Y, -b2, 0;
            C*Y, 0, a2] <= 0;
